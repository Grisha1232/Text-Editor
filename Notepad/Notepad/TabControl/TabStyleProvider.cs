...
namespace System.Windows.Forms
{
	...
	public abstract class TabStyleProvider : Component
	{
		...
		public void PaintTab(int index, Graphics graphics, int tabCount){
			var brush = MainForm.themeLight ? new SolidBrush(Color.FromArgb(255, 255, 255)) : new SolidBrush(Color.FromArgb(45, 45, 45));
			using (GraphicsPath tabpath = this.GetTabBorder(index)) {
				using (Brush fillBrush = brush) {
		...
		private void DrawTabFocusIndicator(GraphicsPath tabpath, int index, Graphics graphics)
		{
			if (this._FocusTrack && this._TabControl.Focused && index == this._TabControl.SelectedIndex) {
				...
				if (!MainForm.themeLight)
                {
					focusBrush = new SolidBrush(Color.FromArgb(45, 45, 45));
                }
